...
fire eight enrolments at once, and against a cold database they all reach
...
pg_extension_name_index and the applicant never got enrolled.
"""
...
def test_the_same_photo_enrols_once():
    ...
    with kycstore.connect() as conn:
        ...
        for _ in range(3):
            kycstore.insert(
                conn, "subject_000", "enrolled/s0/id.jpg", _Array(_vector(0))
            )
        with conn.cursor() as cur:
            cur.execute(
                "SELECT count(*) FROM faces WHERE s3_key = 'enrolled/s0/id.jpg'"
            )
            assert cur.fetchone()[0] == 1


def test_search_returns_the_nearest_subject_first():
    """A probe close to one enrolled face ranks that subject above the others."""
    _fresh_schema()
    with kycstore.connect() as conn:
        kycstore.ensure_schema(conn)
        for i in range(5):
...
def test_search_can_exclude_the_probe_itself():
    ...
    with kycstore.connect() as conn:
        ...
        excluded = kycstore.search(conn, own, k=3, exclude_key="enrolled/s1/id.jpg")
